...
class TestDiaryCreateView(LoggedInTestCase):
    def test_create_diary_success(self):
        ...
        response = self.client.post(reverse_lazy('k_app003:diary_create'), params)
        self.assertRedirects(response, reverse_lazy('k_app003:diary_list'))
        self.assertEqual(Diary.objects.filter(title='テストタイトル').count(), 1)

    def test_create_diary_failure(self):
        response = self.client.post(reverse_lazy('k_app003:diary_create'))
...
class TestDiaryUpdateView(LoggedInTestCase):
    def test_update_diary_success(self):
        ...
        response = self.client.post(reverse_lazy('k_app003:diary_update', kwargs={'pk': diary.pk}), params)
        self.assertRedirects(response,reverse_lazy('k_app003:diary_detail', kwargs={'pk': diary.pk}))
        self.assertEqual(Diary.objects.get(pk=diary.pk).title, 'タイトル編集後')

    def test_update_diary_failure(self):
        response = self.client.post(reverse_lazy('k_app003:diary_update', kwargs={'pk': 999}))
...
class TestDiaryDeleteView(LoggedInTestCase):
    def test_delete_diary_success(self):
        ...
        response = self.client.post(reverse_lazy('k_app003:diary_delete', kwargs={'pk': diary.pk}))
        self.assertRedirects(response, reverse_lazy('k_app003:diary_list'))
        self.assertEqual(Diary.objects.filter(pk=diary.pk).count(), 0)

    def test_delete_diary_failure(self):
        response = self.client.post(reverse_lazy('k_app003:diary_delete', kwargs={'pk': 999}))
        self.assertEqual(response.status_code, 404)
